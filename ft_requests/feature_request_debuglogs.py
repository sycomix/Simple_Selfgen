...
class Feature_Request_DebugLogs:
    ...
    def request_manager(self):
        if self.user_action_debug_or_not():
            ...
            if request_success := self.request_code(args):
    ...
    def get_user_model_and_temp(self):
        while True:
            print()
            print(f"Default Model: {self.common_instance.model[1]} Temperature: {self.common_instance.model_temp}")
            ...
            while True:
                print()
                u_temp = self.common_instance.user_interaction_instance.request_input_from_user("Enter model temperature (float 0-1): ")
                try:
                    u_temp = float(u_temp)
                except Exception as e:
                    print(f"Invalid temperature {u_temp}. Value must be float or int value 0-1.")
                    continue
                if (isinstance(u_temp, (float, int))) and 0 <= u_temp <= 1.0:
